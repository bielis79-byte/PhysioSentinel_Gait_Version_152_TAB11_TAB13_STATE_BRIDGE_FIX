...
def _plot_skel_mesh_walker(mesh_seq):
    ...
    try:
        ...
        if not _ham_status.get("ready"):
            ...
            _cache_key=("v152",int(len(_Sv)),int(_Sv.shape[1]),float(np.nanmean(_Jv[0])),_vh_cache_sha)
                ...
                with st.spinner("Preparando anatomía compacta Denver: una malla + transformaciones de 75 frames…"):
                    ...
                    if _vh_man and _vh_man.get("ready"):
                        try:
                            ...
                            # V149: sustituye el bounded-skinning muscular antiguo por el rig
                            ...
                            try:
                                if _poses_v149.ndim!=2 or len(_poses_v149)!=len(_Jv):
                                    raise RuntimeError("V151 requiere poses SKEL de 75 frames dentro del NPZ/secuencia")
                                _MR=build_all76_rig(
                                ...
                                _MR=None
                                st.warning(
                                    "El rig muscular V151 no pudo construirse; los huesos Denver se conservan y "
                                    "la musculatura vuelve al bounded-skinning anterior. "
                                    f"{type(_mre).__name__}: {_mre}"
                                )
                            # Only the small upper-body Hamner fallback is stored as 75-frame geometry.
                            _upper_ids={
                            ...
                            st.warning(
                                "Denver compacto no pudo construirse; se activa Hamner de respaldo: "
                                f"{type(_ve).__name__}: {_ve}"
                            )
                            _DC=None

                    if _DC is None:
                        # Lazy fallback: full Hamner is built ONLY if Denver is unavailable/failed.
            ...
            if st.session_state.get("v3228_denver_real_bones_active"):
                if _MR is not None:
                    st.success(
                        "Versión 152 · anatomía compacta: huesos Denver conservan el registro landmark de V148 y "
...
def render_skel_poc_panel(motion, acquisition_mode="monocular_depth_safe", acquisition_label=None):
    ...
    st.caption("Versión 152 · Carga el NPZ descargado de este registro para abrir directamente sus 75 frames SKEL. No repite fitting, retargeting ni generación de los 75 frames.")
